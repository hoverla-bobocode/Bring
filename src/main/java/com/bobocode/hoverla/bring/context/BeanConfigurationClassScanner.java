...
public class BeanConfigurationClassScanner implements BeanScanner {
    ...
    public List<BeanDefinition> scan() {
        ...
        return configClasses.stream()
                .map(this::scanBeanDefinitions)
                .flatMap(List::stream)
                .toList();
    }

    private List<BeanDefinition> scanBeanDefinitions(Class<?> configClass) {
        validator.validate(configClass);
        Object configInstance = getConfigInstance(configClass);

        return getBeanMethods(configClass).stream()
                .map(method -> mapper.mapToBeanDefinition(configInstance, method))
                .toList();
    }

    private List<Method> getBeanMethods(Class<?> configClass) {
